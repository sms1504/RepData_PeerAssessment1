...
- Steps per Day (mean):   `r sprintf("%.2f", stepsPerDay.mean)`.
- Steps per Day (median): `r sprintf("%.0f", stepsPerDay.median)`.
...
This indicates that there are `r steps.missing` incomplete records.
...
So there are `r steps.imputed.missing` incomplete records in the imputed array.
...
- Steps per Day (mean):   `r sprintf("%.2f", stepsPerDay.imputed.mean)`.
- Steps per Day (median): `r sprintf("%.0f", stepsPerDay.imputed.median)`.
...
qplot(interval, steps, data = intervalDayType, facets = dayType ~ ., size=I(0)) + geom_line()
```
